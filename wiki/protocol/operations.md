...
共享 Database 下设 10 个资源目录。每个目录可以同时包含原创内容和 curated（非原创整理）内容——**目录按内容类型分类，不按原创/非原创分类**。一份投稿属于哪个目录，取决于它是什么，而非它是谁写的。

- `motions/` — 辩题库
- `methods/` — 训练方法论、练习设计、技战术分析
- `transcripts/` — 比赛文字转写、录音整理
- `ai-tools/` — AI coach 使用方法与 prompt 分享
- `casefiles/` — 辩题案例文件、证据卡片
- `english-learning/` — 英语学习方法、材料、工具推荐
- `news/` — 辩论议题相关新闻分析或新闻 digest
- `curated-video/` — 非原创视频资源整理
- `curated-audio/` — 非原创音频资源整理
- `curated-text/` — 非原创文字资源整理

投稿的类型归属由内容特征决定（见 dcp-ingest 工作流中的分类表），投稿的来源性质（原创 / curated）通过页面的 `source_type` frontmatter 字段标注，不通过目录区分。Curatorial 投稿必须标注原始来源与版权归属——缺来源标注直接退回。
...
| 违反 CC BY-SA 4.0 许可条款              | 首次：告知函 + 公开违规记录。持续不纠正 → 若违规者为成员 → Banished；若违规者非成员 → 永久禁止入会 |
