...
```
wiki/
  members/         — 每位成员独立页面，含 Badge 状态、贡献记录、联系方式
    _index.md      — 成员全景索引（可查证所有成员状态）
  protocol/        — 章程正文、本运营手册、规则修改提案历史
  resources/       — 共享 Database 中所有资源的索引页
  events/          — 赛事与活动日历
...
LLM-Wiki 托管在公开 Git 仓库中。具体托管平台与技术栈由协议技术维护者与 Maintainer 成员共同决定。当前参照方案：GitHub + GitHub Pages + 协议接入端桥接。平台迁移须经全体 Maintainer 投票通过。
...
成员通过协议接入渠道提交资源 → AI 自动预处理并生成 Wiki 页面草稿 + 质量初评 → 草稿进入审核队列。
